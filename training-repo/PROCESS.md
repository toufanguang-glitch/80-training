...
用瀏覽器自動化建立訂單、截圖結果頁,對照活動 1 練習 2 人工重現 bug 的步驟:
人工要一步步找表單欄位、填值、送出、再手動截圖確認;有了工具之後這些步驟
還是要做,但不用我自己讀 HTML 猜欄位名稱——agent 自己導覽、填表、驗證結果。
差別不是「不用做」,而是「不用我逐步下每個指令,agent 自己串起完整流程」。
